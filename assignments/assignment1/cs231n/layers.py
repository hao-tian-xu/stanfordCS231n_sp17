...
def svm_loss(x, y):
    ...
    correct_class_x = x[np.arange(num_train), y].reshape((num_train, 1))
    
    margin = np.maximum(0, x - correct_class_x + 1)
...
def softmax_loss(x, y):
    ...
    # # memo: try to solve the 'RuntimeWarning: divide by zero encountered in log' problem
    # # this occurs with large learning rate like 1e-2
    # boundary = 1e-10
    # softmax[softmax < boundary] = boundary
