...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # out, cache_af1 = affine_forward(X, W1, b1)
        # out, cache_relu = relu_forward(out)
        # scores, cache_af2 = affine_forward(out, W2, b2)
        # memo: use Sandwich Layers
        ...
        # softmax loss and gradients with regularization
        loss, dx = softmax_loss(scores, y)
        loss += reg * 0.5 * (np.sum(W1 * W1) + np.sum(W2 * W2))
        # # 2nd affine layer
        # dx, dw, db = affine_backward(dx, cache_af2)
        # grads['W2'] = dw + reg * W2
        # grads['b2'] = db
        # # relu layer
        # dx = relu_backward(dx, cache_relu)
        # # 1st affine layer
        # dx, dw, db = affine_backward(dx, cache_af1)
        # grads['W1'] = dw + reg * W1
        # grads['b1'] = db
        # memo: use Sandwich Layers
        # 2nd affine layer
