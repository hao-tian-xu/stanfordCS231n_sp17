...
def softmax_loss(x, y):
    ...
    probs = np.exp(scores)
    softmax = np.divide(probs, np.sum(probs, axis=1, keepdims=True))
...
def layernorm_backward(dout, cache):
    ...
    dxhat = dout * gamma
    dx = ((1. / (D * np.sqrt(sample_var + eps))) * (
                D * dxhat.T - np.sum(dxhat, axis=1) - x_hat.T * np.sum(dxhat * x_hat, axis=1))).T
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # apply filters and add to output
    for i in range(0, H + 2 * pad - HH + 1, stride):
        for j in range(0, W + 2 * pad - WW + 1, stride):
            # a certain receptive field of x (in all channels and filters)
            # add an intermediate filter dimension (1) to broadcast with w
            x_recep = x_pad[:, :, i:i + HH, j:j + WW].reshape((N, 1, C, HH, WW))
            out[:, :, i // stride, j // stride] = np.sum(x_recep * w, axis=(2, 3, 4)) + b
    # # v1
    # for n in range(N):
    #     for f in range(F):
    #         for i in range(0, H + 2 * pad - HH + 1, stride):
    #             for j in range(0, W + 2 * pad - WW + 1, stride):
    #                 x_recep = x_pad[n, :, i:i + HH, j:j + WW]
    #                 out[n, f, i // stride, j // stride] = np.sum(x_recep * w[f]) + b[f]
    # # v2
    # for n in range(N):
    #     for i in range(0, H + 2 * pad - HH + 1, stride):
    #         for j in range(0, W + 2 * pad - WW + 1, stride):
    #             x_recep = x_pad[n, :, i:i + HH, j:j + WW]
    #             out[n, :, i // stride, j // stride] = np.sum(x_recep * w, axis=(1, 2, 3)) + b
    #             print(x_recep.shape, w.shape, b.shape)
    pass
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(0, H + 2 * pad - HH + 1, stride):
        for j in range(0, W + 2 * pad - WW + 1, stride):
            x_recep = x_pad[:, :, i:i + HH, j:j + WW].reshape((N, 1, C, HH, WW))
            dx_recep = dx_pad[:, :, i:i + HH, j:j + WW]
            dout_recep = dout[:, :, i // stride, j // stride].reshape((N, F, 1, 1, 1))
            
            dx_recep += (w * dout_recep).sum(axis=1)
            dw += (x_recep * dout_recep).sum(axis=0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    out = np.zeros((N, C,
                    1 + (H - pool_height) // stride,
                    1 + (W - pool_width) // stride))
    
    for i in range(0, H - pool_height + 1, stride):
        for j in range(0, W - pool_width + 1, stride):
            x_recep = x[:, :, i:i + pool_height, j:j + pool_width]
            out[:, :, i // stride, j // stride] = np.max(x_recep, axis=(2, 3))
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(0, H - pool_height + 1, stride):
        for j in range(0, W - pool_width + 1, stride):
            x_recep = x[:, :, i:i + pool_height, j:j + pool_width].reshape((N * C, -1))
            dout_recep = dout[:, :, i // stride, j // stride].reshape(-1)
            dx_recep = dx[:, :, i:i + pool_height, j:j + pool_width]
            
            temp = np.zeros((N * C, pool_height * pool_width))
            temp[np.arange(N * C), np.argmax(x_recep, axis=1)] += dout_recep
            dx_recep += temp.reshape((N, C, pool_height, pool_height))
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    _x = np.reshape(np.transpose(x, (0, 2, 3, 1)), (N * H * W, C))
    _out, cache = batchnorm_forward(_x, gamma, beta, bn_param)
    out = np.transpose(np.reshape(_out, (N, H, W, C)), (0, 3, 1, 2))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    _dout = np.reshape(np.transpose(dout, (0, 2, 3, 1)), (N * H * W, C))
    _dx, dgamma, dbeta = batchnorm_backward(_dout, cache)
    dx = np.transpose(np.reshape(_dx, (N, H, W, C)), (0, 3, 1, 2))
