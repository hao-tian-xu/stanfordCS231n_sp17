...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2 * N):
            z_i = out[i]
            if i != k:
                denom_k += torch.exp(sim(z_k, z_i) / tau)
            if i != k + N:
                denom_k_N += torch.exp(sim(z_k_N, z_i) / tau)
        loss_k = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / denom_k)
        loss_k_N = -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / denom_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    left_norm = out_left / torch.norm(out_left, dim=1, keepdim=True)
    right_norm = out_right / torch.norm(out_right, dim=1, keepdim=True)
    pos_pairs = torch.sum(left_norm * right_norm, dim=1, keepdim=True)
...
def compute_sim_matrix(out: torch.Tensor):
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sims = sim_positive_pairs(out_left, out_right).to(device)
    sims = torch.concat([sims, sims])
