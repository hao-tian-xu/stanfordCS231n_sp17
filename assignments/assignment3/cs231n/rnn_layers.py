...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_t = h0
    for t in range(T):
        h_t, cache[t] = rnn_step_forward(x[:, t, :], h_t, Wx, Wh, b)
        h[:, t, :] = h_t
    cache.append(D)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dx[:, t, :], dh_t_backward, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
        dh_t = dh[:, t - 1, :] + dh_t_backward if t > 0 else dh_t_backward
    dh0 = dh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    gates = sigmoid(temp[:, :3 * H])
    i, f, o = np.split(gates, 3, axis=1)
    curr_c = np.tanh(temp[:, 3 * H:])
    
    next_c = f * prev_c + i * curr_c
    next_h = o * np.tanh(next_c)
    
    cache = (x, prev_h, prev_c, next_c, next_h, Wx, Wh, temp, gates, i, f, o, curr_c, H)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, next_c, next_h, Wx, Wh, temp, gates, i, f, o, curr_c, H = cache
    dnext_c += dnext_h * o * (1 - np.tanh(next_c) ** 2)
    do = dnext_h * np.tanh(next_c)
    df = dnext_c * prev_c
    di = dnext_c * curr_c
    dgates = np.concatenate((di, df, do), axis=1) * sigmoid(temp[:, :3 * H]) * (1 - sigmoid(temp[:, :3 * H]))
    dcurr_c = dnext_c * i
    dtemp_curr_c = dcurr_c * (1 - np.tanh(temp[:, 3 * H:]) ** 2)
    dtemp = np.concatenate((dgates, dtemp_curr_c), axis=1)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    c_t = np.zeros((N, H))
    for t in range(T):
        h_t, c_t, cache[t] = lstm_step_forward(x[:, t, :], h_t, c_t, Wx, Wh, b)
        h[:, t, :] = h_t
    cache.append(D)
...
def lstm_backward(dh, cache):
    ...
    dc_t = np.zeros((N, H))
    for t in reversed(range(T)):
        dx[:, t, :], dh_t_backward, dc_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh_t, dc_t, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
        dh_t = dh[:, t - 1, :] + dh_t_backward if t > 0 else dh_t_backward
