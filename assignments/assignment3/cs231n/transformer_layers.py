...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        i = torch.arange(0, max_len).view(-1, 1)
        denom = 10000 ** (torch.arange(0, embed_dim, 2) / embed_dim)
        pe[:, :, 0::2] = torch.sin(i / denom)
        pe[:, :, 1::2] = torch.cos(i / denom)
        
        # # naive implementation
        # for i in range(max_len):
        #     for j in range(embed_dim):
        #         pe[:, i, j] = math.sin(i * 10000 ** (-j / embed_dim)) if j % 2 == 0 \
        #             else math.cos(i * 10000 ** (-(j - 1) / embed_dim))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        V: torch.Tensor = self.value(value).view(N, T, H, D // H)
        K: torch.Tensor = self.key(key).view(N, T, H, D // H)
        Q: torch.Tensor = self.query(query).view(N, S, H, D // H)
        ...
        attn_distr = F.dropout(F.softmax(attn_score, dim=-1), self.dropout)
        
        attn_output = torch.matmul(attn_distr, V.permute(0, 2, 1, 3))\
            .permute(0, 2, 1, 3).reshape(N, S, D)
